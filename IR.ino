...
int DIR_L_B = 7;    // Left Direction pin that will indicate backwards movement (1 for backwards, 0 for forewards).
int PWM_L = 9;      // Speed controll pin. *** This pin must be plugged into an output pin on the arduino that is labled PWM ***.

int DIR_R_F = 6;     // Right Direction pin that will indicate forewards movement (1 for forewards, 0 for backwards)
int DIR_R_B = 5;     // Left Direction pin that will indicate backwards movement (1 for backwards, 0 for forewards).
int PWM_R = 12;       // Speed controll pin. *** This pin must be plugged into an output pin on the arduino that is labled PWM ***.
...
double pingTime; 
double DIST_SCALE = 1.085767; 
double SPEED_OF_SOUND = 331.0;
...
//
void detectColor() {
  int input1 = digitalRead(IRinput1);
  int input2 = digitalRead(IRinput2);
  // float MotorOutputs[4];       // Left Direction, Right Direction, Left Speed, Right Speed.
...
void ultrasonicDistance(){
  ...
  double dist = DIST_SCALE*0.5*(SPEED_OF_SOUND*(pingTime/convert)); 
